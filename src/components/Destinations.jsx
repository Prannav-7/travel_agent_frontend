                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {destinations.map((dest, index) => (
                        ...
                        >
                            ...
                            {/* Gradient Overlay */}
                            <div className={`absolute inset-0 bg-gradient-to-t ${dest.gradient} opacity-60 group-hover:opacity-80 transition-opacity duration-300`}></div>

                            {/* Content */}
                            <div className="absolute inset-0 p-6 flex flex-col justify-between">
                                {/* Badge */}
                                <div className="self-start">
                                    <span className="px-3 py-1 rounded-full glass text-sm font-semibold backdrop-blur-xl">
                                        {dest.badge}
                                    </span>
                                </div>
                                ...
                                <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-3xl font-bold text-gradient from-white to-gray-300">
                                            From {dest.price}
                                        </span>
                                        <button className="px-4 py-2 bg-white/20 backdrop-blur-xl rounded-full text-sm font-semibold hover:bg-white/30 transition-all opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0">
                                            Explore →
                                        </button>
                                    </div>
                                </div>
